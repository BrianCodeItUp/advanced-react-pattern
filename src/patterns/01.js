...
const MediumClap = ({ animate }) => {
  ...
  const handleClapClick = () => {
    animate()
    // set state
    setClapState(prevStae => ({ 
      isClicked: true,
      count: Math.min(prevStae.count+1, MAXIMUM_USER_CLAP),
      countTotal: prevStae.count < MAXIMUM_USER_CLAP 
        ? prevStae.countTotal +1 
        :  prevStae.countTotal
    }))    
  }
...
const Usage = () => {
  const AnimationMediumClap = withClapAnimation(MediumClap);
  return <AnimationMediumClap />;
}
